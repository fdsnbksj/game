...
- `src/runStore.ts` (zustand) holds the run in progress and saves it to `localStorage`, so a reload resumes it. The scene subscribes to it as its `BoardStore` (see below).
...
- **One board, several modes:** the board scene and the shared parts in `src/screens/board.tsx` read a `BoardStore` (`src/boardStore.ts`), not the run store: the scene through the Phaser registry (`store`), React through `BoardStoreContext`, both defaulting to `useRunStore`. A mode that isn't a run supplies its own zustand store of that shape.
- **Battle puzzles** (`/puzzle`): `puzzle(level)` in `src/sim/puzzle.ts` makes a level from its number alone: a placed rival and a hand to place. It tunes the hand until about `targetShare(level)` of sampled placements win and keeps a winning one, so every level can be won. A level is played as a `RunState` built from the hand (bench = hand, no shop), so the planning reducers and scene work unchanged; the rival shows through `rivalPreview`. Changing the generator or anything in combat changes every level: bump `PUZZLE_VERSION` (combat changes already bump `BALANCE_VERSION`); `LADDER_VERSION` combines both and `puzzleVersion()` in `firestore.rules` must match it (`rulesSync.test.ts` checks).
- **The puzzle ladder** is `puzzles/{uid}_{LADDER_VERSION}`: the highest level cleared and each level's winning layout. Clears are queued in `src/puzzleStore.ts` (localStorage `game:puzzle`) and written one level at a time, at least 3 s apart; the first one creates the doc at level 1. The rules check layouts' shape only, so `npm run audit` replays them.
...
- **Catching cheats:** the rules can't replay a fight, so `npm run audit` does it here. Every board records the rival it fought, so `scripts/auditRuns.ts` re-simulates each round and reports runs whose wins or HP don't match; `scripts/auditPuzzles.ts` remakes each puzzle level and replays the ladder's solutions. Read-only; run it by hand against production with `AUDIT_PROJECT_ID=<id>`.
