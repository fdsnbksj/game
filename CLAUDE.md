...
- Creatures are shape data in `src/shared/creatureShapes.ts`, baked into Phaser textures and drawn as SVG by `CreatureChip`, so every view shows the same creature. Creature art is generated in code; the only image files are the app icons in `public/`.
