...
- **Starter sets** (`src/learn/starters.ts`) are one-tap sets of a book's key ideas, currently *Make Something Wonderful*. They must be written in our own words, never the book's text, because the books are copyrighted; exact lines come only from a player's own highlights.
